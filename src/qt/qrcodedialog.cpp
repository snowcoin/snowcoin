...
{
    QString ret = QString("snowcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non SNC unit input in lnReqAmount, we generate the URI with SNC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(snowcoinUnits::format(snowcoinUnits::SNC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
