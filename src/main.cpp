...
unsigned int nStakeTargetSpacing = 2 * 60; // 120 seconds block spacing
const int64 nChainStartTime = 1386403023; 
...
const string strMessageMagic = "snowcoin Signed Message:\n";
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal snowcoin miner
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in snowcoin
    bool fStrictPayToScriptHash = true; // Always active in snowcoin
    ...
    // ppcoin: fees are not collected by miners as in snowcoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "snowcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "snowcoin - grab em all.";
        CTransaction txNew;
        ...
        txNew.strTxComment = "text:snowcoin genesis block";
        ...
        assert(block.hashMerkleRoot == uint256("0xed65789d4970e00eca66f0b8d957e9160366954ac0356c88cf9c6a0b9a38abcf"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("snowcoinMiner : proof-of-work not meeting target");

    //// debug print
    printf("snowcoinMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("snowcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("snowcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadsnowcoinMiner(void* parg);

static bool fGeneratesnowcoins = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void snowcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    printf("CPUMiner started for proof-of-%s\n", fProofOfStake? "stake" : "work");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);

    // Make this thread recognisable as the mining thread
    RenameThread("snowcoin-miner");

    // Each thread has its own key and counter
    CReserveKey reservekey(pwallet);
    unsigned int nExtraNonce = 0;

    while (fGeneratesnowcoins || fProofOfStake)
    {
        if (fShutdown)
            return;
        while (vNodes.empty() || IsInitialBlockDownload())
        {
            ...
            if (fShutdown)
                return;
            if ((!fGeneratesnowcoins) && !fProofOfStake)
                return;
        }
        ...
        printf("Running snowcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadsnowcoinMiner(void* parg)
{
    ...
    try
    {
        ...
        snowcoinMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadsnowcoinMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadsnowcoinMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadsnowcoinMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void Generatesnowcoins(bool fGenerate, CWallet* pwallet)
{
    fGeneratesnowcoins = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGeneratesnowcoins = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d snowcoinMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadsnowcoinMiner, pwallet))
                printf("Error: NewThread(ThreadsnowcoinMiner) failed\n");
            Sleep(10);
        }
    }
}
